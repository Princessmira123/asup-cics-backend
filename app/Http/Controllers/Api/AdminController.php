...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        return response()->json([
            ...
            'stats'   => [
                ...
                'pending_verifications'  => Member::where('status', 'pending_verification')->count(),
    ...
    // necessary escape hatch: without it, a member with an undelivered OTP
    // has no way to ever activate their account.
    public function verifyMember($id)
    {
        $member = Member::where('member_id', $id)->firstOrFail();
        if ($member->status !== 'pending_verification') {
            return response()->json(['success' => false, 'message' => 'This member is not pending verification.'], 400);
        }
        $member->update(['status' => 'active']);
        return response()->json(['success' => true, 'message' => 'Member manually verified and activated.']);
    }
